...
export async function extendSession(id: string, role: Role): Promise<SessionWithProfile | null> {
  try {
    ...
    // Session bestaat niet (oude cookie / al verwijderd) -> geen crash
    return null
  }
}
...
export async function adminUpdateUser(id: string, data: {username: string; role: Role}) {
  return prismaClient.user.update({
    where: {id},
    data,
  })
...
export async function adminDeleteUser(id: string) {
  // loskoppelen van boulders die door user gezet zijn (anders faalt delete)
  await prismaClient.boulder.updateMany({
    where: {setByUserId: id},
