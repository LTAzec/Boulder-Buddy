...
export async function redirectProxy(
  ...
  logger.info(`Accessing route: ${request.nextUrl.pathname} (mapped to ${parameterizedRoute})`)
  ...
  //Protect ALL admin routes in one go
  if (parameterizedRoute.startsWith('/admin')) {
